# Shared helpers for Vacancy-section production visuals.
#
# This file centralizes the repeated work every standalone visual needs:
# - resolve repo-relative paths
# - connect to the materialized DuckDB
# - load shared visual-library helpers
# - run local section SQL files
# - save deliberate PNG outputs
#
# The goal is to keep each chart script focused on one question and one output
# while still using a consistent production workflow across the section.
...
vacancy_get_script_path <- function(default_path) {
...
vacancy_build_context <- function(script_path) {
...
vacancy_read_sql <- function(path) {
  paste(readLines(path, warn = FALSE), collapse = "\n")
}

vacancy_with_foundations_wd <- function(foundations_dir, expr) {
  old_wd <- getwd()
...
vacancy_load_visual_library <- function(foundations_dir, chart_types) {
  vacancy_with_foundations_wd(foundations_dir, {
...
vacancy_connect_duckdb <- function(db_path) {
...
vacancy_run_sql_file <- function(con, sql_dir, filename) {
  DBI::dbGetQuery(con, vacancy_read_sql(file.path(sql_dir, filename)))
}

vacancy_save_plot <- function(plot, output_path, width, height) {
...
vacancy_load_contiguous_states_context <- function(con) {
  states_sql <- paste(
...
vacancy_load_contiguous_us_outline <- function(states_sf) {
