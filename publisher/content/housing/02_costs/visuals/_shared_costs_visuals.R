# Shared helpers for Costs-section production visuals.
#
...
costs_get_script_path <- function(default_path) {
...
costs_build_context <- function(script_path) {
...
costs_read_sql <- function(path) {
  paste(readLines(path, warn = FALSE), collapse = "\n")
}

costs_with_foundations_wd <- function(foundations_dir, expr) {
  old_wd <- getwd()
...
costs_load_visual_library <- function(foundations_dir, chart_types = character()) {
  costs_with_foundations_wd(foundations_dir, {
...
costs_connect_duckdb <- function(db_path) {
...
costs_run_sql_file <- function(con, sql_dir, filename) {
  DBI::dbGetQuery(con, costs_read_sql(file.path(sql_dir, filename)))
}

costs_save_plot <- function(plot, output_path, width, height) {
...
costs_load_contiguous_states_context <- function(con) {
  states_sql <- paste(
...
costs_load_contiguous_us_outline <- function(states_sf) {
