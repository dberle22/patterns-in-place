# Shared helpers for Overheating production visuals.
#
...
overheating_get_script_path <- function(default_path) {
...
overheating_build_context <- function(script_path) {
...
overheating_read_sql <- function(path) {
  paste(readLines(path, warn = FALSE), collapse = "\n")
}

overheating_with_foundations_wd <- function(foundations_dir, expr) {
  old_wd <- getwd()
...
overheating_load_visual_library <- function(foundations_dir, chart_types = character()) {
  overheating_with_foundations_wd(foundations_dir, {
...
overheating_connect_duckdb <- function(db_path) {
...
overheating_run_sql_file <- function(con, sql_dir, filename) {
  DBI::dbGetQuery(con, overheating_read_sql(file.path(sql_dir, filename)))
}

overheating_save_plot <- function(plot, output_path, width, height) {
...
overheating_load_contiguous_states_context <- function(con) {
  states_sql <- paste(
...
overheating_load_contiguous_us_outline <- function(states_sf) {
