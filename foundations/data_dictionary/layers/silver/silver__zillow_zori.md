# Data Dictionary: silver.zillow_zori

## Overview
- **Table**: `silver.zillow_zori`
- **Purpose**: Standardized monthly Zillow Observed Rent Index table for county, ZCTA, and derived CBSA geographies.
- **Row count**: 516,933
...
| `zori` | `DOUBLE` | 0.0000 | Zillow Observed Rent Index monthly level for the geography and month. |
...
- Silver now keeps only the last 10 calendar years and drops null-value rows, so this table retains only observed Zillow rent series points instead of a dense geography-month panel.
...
## Lineage
1. `foundations/etl/staging/get_zillow.R` downloads Zillow county and ZIP ZORI files and pivots them into monthly staging tables.
2. `foundations/etl/silver/zillow_silver.R` standardizes county and ZCTA rows, rebases county rows to CBSA using `silver.xwalk_cbsa_county` and county `hu_totalE` weights from `silver.housing_base`, filters to 2016+ non-null observations, and writes `silver.zillow_zori`.
...
- The source is still sparse in time and geography, but Silver now encodes that sparsity by omitting missing months rather than storing null-value rows.
