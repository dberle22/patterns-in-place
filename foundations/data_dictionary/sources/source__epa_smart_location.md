...
- Current implementation note: the modeled Silver contract now builds county rows directly from the CSV and derives CBSA and state rollups from that county base. The direct CSV is sufficient for those geographies because `STATEFP` and `COUNTYFP` survive delivery intact, but tract-level recovery is still not reliable from the CSV alone because the delivered `GEOID10` / `GEOID20` values are scientific-notation strings and a meaningful share of reconstructed tract IDs do not resolve cleanly against the current tract backbone. A future tract fix should use the official Census 2010/2020 tract relationship files or the geodatabase-based SLD delivery.
