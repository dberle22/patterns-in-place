"""Richmond-first Overture POI ingestion for Industry D4 review."""
...
def _merge_manifest(existing: dict | None, new_manifest: dict) -> dict:
    """Merge Overture layer results into an existing combined manifest if present."""
    if not existing:
        return new_manifest
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Parse CLI args for the Richmond Overture run."""
    parser = argparse.ArgumentParser(description="Extract Richmond Overture POIs for D4 review.")
    parser.add_argument(
        "--bbox",
...
def main(argv: list[str] | None = None) -> int:
    """Run the Richmond Overture extraction and update the combined manifest."""
