...
**Build status:** D6 is now live as a dedicated `industry/` page with a sector/occupation toggle. First-pass sector exposure uses the section-owned Felten workbook at `metro-deep-dive/metro-area-explorer/industry/reference_data/AIOE_DataAppendix.xlsx`, joins Appendix B to 4-digit QCEW industry groups rolled from `staging.bls_qcew_county` through `silver.xwalk_cbsa_county`, then collapses those results back to the broad D1 sector taxonomy for comparability. The occupation companion joins Appendix A directly to detailed `silver.bls_oews` SOC rows for `2025`, adds a ranked table plus a bubble-scatter companion, and uses `gold.economics_occupation_wide` for the lighter family summary. Coverage notes stay explicit because both joins inherit static Felten-vintage limits: sector coverage is improved with a small explicit NAICS fallback map for known code revisions and aggregate appendix codes, while occupation coverage remains a first-pass SOC-vintage constraint rather than a silently invented crosswalk.
