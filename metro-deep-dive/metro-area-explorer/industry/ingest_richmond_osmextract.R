...
# Richmond-first OSM infrastructure ingestion via osmextract.
#
# This is a provider-backed alternative to the unstable Overpass path and the
# oversized Virginia-scale pyrosm download. The current prototype targets the
# Richmond extract exposed by the openstreetmap_fr provider, then filters down
# to the infrastructure families we care about for D4 review.
...
section_root <- normalizePath("metro-deep-dive/metro-area-explorer/industry", mustWork = TRUE)
output_dir <- file.path(section_root, "outputs", "richmond_va_osmextract")
