"""Build the road overlay map product."""
...
from data_builds.maps.shared import build_road_geojson
from site_prep import get_default_site_config_path, list_site_configs, load_site


def build_map_roads(site_config_path: str, artifact_dir: Path):
    """Build `map/roads.geojson` for one site."""

    site = load_site(site_config_path)
    geojson, elapsed = time_step(site.site_id, "map.roads", lambda: build_road_geojson(site))
    map_dir = artifact_dir / "map"
    map_dir.mkdir(parents=True, exist_ok=True)
    write_json(map_dir / "roads.geojson", geojson)
    record_manifest_step(site, artifact_dir, site_config_path, "map_roads", elapsed)
    return geojson, elapsed


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Place Intelligence road map artifacts.")
...
def main() -> int:
    ...
    for site_config in site_configs:
        ...
        build_map_roads(site_config, artifact_dir)
        print(f"Built map roads for {Path(site_config).name} -> {artifact_dir}")
    return 0
