"""Build the D3 node typology site metadata from prior D3 products."""
...
from data_builds.d3.shared import build_node_typology_meta
from site_prep import get_default_site_config_path, list_site_configs, load_site


def build_d3_node_typology(site_config_path: str, artifact_dir: Path):
    """Build the late-stage D3 node typology metadata from prior site outputs."""

    site = load_site(site_config_path)
    require_artifact_files(artifact_dir, ["d3_daytime_population.csv", "d3_poi_counts.csv", "d3_road_context.json"], "d3.node_typology")
    daytime = read_dataframe(artifact_dir / "d3_daytime_population.csv")
    poi_counts = read_dataframe(artifact_dir / "d3_poi_counts.csv")
    road_context = read_json(artifact_dir / "d3_road_context.json")
    payload, elapsed = time_step(site.site_id, "d3.node_typology", lambda: build_node_typology_meta(daytime, poi_counts, road_context))
    write_json(artifact_dir / "d3_meta.json", payload)
    record_manifest_step(site, artifact_dir, site_config_path, "d3_node_typology", elapsed)
    return payload, elapsed


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Place Intelligence D3 node typology artifacts.")
...
def main() -> int:
    ...
    for site_config in site_configs:
        ...
        build_d3_node_typology(site_config, artifact_dir)
        print(f"Built D3 node typology artifact for {Path(site_config).name} -> {artifact_dir}")
    return 0
