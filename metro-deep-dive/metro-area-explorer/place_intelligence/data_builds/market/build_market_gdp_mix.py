"""Build the Market GDP-mix product."""
...
def build_market_gdp_mix(site_config_path: str, artifact_dir: Path):
    """Build `market_gdp_mix.csv` for one site."""

    site = load_site(site_config_path)
    payload, elapsed = time_step(site.site_id, "market.gdp_mix", lambda: build_market_payload(site))
    write_dataframe(artifact_dir / "market_gdp_mix.csv", normalize_market_frame(payload["industry_context"]["gdp_mix"]))
    record_manifest_step(site, artifact_dir, site_config_path, "market_gdp_mix", elapsed)
    return payload, elapsed


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Place Intelligence market GDP-mix artifacts.")
...
def main() -> int:
    ...
    for site_config in site_configs:
        ...
        build_market_gdp_mix(site_config, artifact_dir)
        print(f"Built market GDP mix for {Path(site_config).name} -> {artifact_dir}")
    return 0
