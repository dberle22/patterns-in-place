"""Build the Market employment-mix product."""
...
def build_market_employment_mix(site_config_path: str, artifact_dir: Path):
    """Build `market_employment_mix.csv` for one site."""

    site = load_site(site_config_path)
    payload, elapsed = time_step(site.site_id, "market.employment_mix", lambda: build_market_payload(site))
    write_dataframe(artifact_dir / "market_employment_mix.csv", normalize_market_frame(payload["industry_context"]["employment_mix"]))
    record_manifest_step(site, artifact_dir, site_config_path, "market_employment_mix", elapsed)
    return payload, elapsed


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Place Intelligence market employment-mix artifacts.")
...
def main() -> int:
    ...
    for site_config in site_configs:
        ...
        build_market_employment_mix(site_config, artifact_dir)
        print(f"Built market employment mix for {Path(site_config).name} -> {artifact_dir}")
    return 0
