"""Build the D5 NRI score and benchmark site outputs from staged market NRI rows."""
...
def build_d5_nri_scores(site_config_path: str, artifact_dir: Path):
    """Build the NRI catchment scores and CBSA benchmark from staged market NRI rows."""

    site = load_site(site_config_path)
    market_dir = ensure_market_artifact_dir(site.market_id)
    require_artifact_files(market_dir, ["d5_market_nri_inputs.csv"], "d5.nri_scores")
    require_artifact_files(artifact_dir, ["base_weight_table.csv"], "d5.nri_scores")
    ...
    payload, elapsed = time_step(site.site_id, "d5.nri_scores", lambda: build_nri_scores_payload(site, weight_table, nri_surface))
    write_dataframe(artifact_dir / "d5_nri_catchment_scores.csv", payload["catchment_scores"])
    write_dataframe(artifact_dir / "d5_nri_cbsa_benchmark.csv", payload["cbsa_benchmark"])
    record_manifest_step(site, artifact_dir, site_config_path, "d5_nri_scores", elapsed)
    return payload, elapsed


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Place Intelligence D5 NRI score artifacts.")
...
def main() -> int:
    ...
    for site_config in site_configs:
        ...
        build_d5_nri_scores(site_config, artifact_dir)
        print(f"Built D5 NRI score artifacts for {Path(site_config).name} -> {artifact_dir}")
    return 0
