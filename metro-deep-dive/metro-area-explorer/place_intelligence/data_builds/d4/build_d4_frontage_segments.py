"""Build the D4 frontage segment site output from staged market segments."""
...
from data_builds.d4.shared import build_frontage_segments_frame, read_geojson_frame
from site_prep import get_default_site_config_path, list_site_configs, load_site


def build_d4_frontage_segments(site_config_path: str, artifact_dir: Path):
    ...
    require_artifact_files(market_dir, ["d4_market_current_segments.geojson"], "d4.frontage_segments")
    current_segments = read_geojson_frame(market_dir / "d4_market_current_segments.geojson")
    frame, elapsed = time_step(site.site_id, "d4.frontage_segments", lambda: build_frontage_segments_frame(site, current_segments))
    write_dataframe(artifact_dir / "d4_frontage_segments.csv", frame)
    record_manifest_step(site, artifact_dir, site_config_path, "d4_frontage_segments", elapsed)
    return frame, elapsed


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Place Intelligence D4 frontage segment artifacts.")
...
def main() -> int:
    ...
    for site_config in site_configs:
        ...
        build_d4_frontage_segments(site_config, artifact_dir)
        print(f"Built D4 frontage segment artifact for {Path(site_config).name} -> {artifact_dir}")
    return 0
