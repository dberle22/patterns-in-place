...
- Subject decisions from the notebook:
  - `Affordability`: keep
  - `Health`: keep
  - `Safety`: keep, but thin
  - `Access & Infrastructure`: keep
  - `Education access`: drop from the Phase 1 core
  - `Physical Environment`: drop from the Phase 1 core and treat as sensitivity-only for now
...
**Why keep it**
- Health remains one of the most defensible non-affordability parts of Livability.
- Average topic coverage is `97.9%`.
- The two live health topics behave differently enough to justify keeping both.

**Contrarian take**
- Health can drift into a broad distress frame rather than daily-life livability if too many mortality and deprivation measures stay in the core.
...
**Subject decision:** Keep, but thin

**Why keep it**
- Average topic coverage is `98.0%`.
- The subject is analytically real, but it is only one topic deep in the current Livability frame.

**Contrarian take**
- Safety may end up behaving more like a sub-score inside Health than a standalone subject if we keep the frame compact.
- It stays separate for now because the lived-risk story is distinct enough.
