phase4_has_year_column <- function(con, source_table) {
  sql <- sprintf(
    "select count(*) as n
...
phase4_pull_metric <- function(con, metric_id, source_table, source_column) {
  if (phase4_has_year_column(con, source_table)) {
    sql <- sprintf(
      "with ranked as (
         select
           geo_id,
...
build_phase4_opportunity_frame <- function(con, catalog_check) {
  ...
  opportunity_frame <- spine |>
    dplyr::left_join(metric_frame, by = "cbsa_code")

  stopifnot(nrow(opportunity_frame) == 396)
  opportunity_frame
}
