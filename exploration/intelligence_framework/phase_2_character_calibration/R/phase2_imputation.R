build_phase2_imputation_bundle <- function(character_frame, catalog_check, config) {
  ...
  metric_completeness <- config$expected_kpis |>
    dplyr::mutate(
      non_null_cbsas = purrr::map_int(metric_id, \(metric) sum(!is.na(character_frame[[metric]]))),
      missing_cbsas = nrow(character_frame) - non_null_cbsas,
      completeness_pct = non_null_cbsas / nrow(character_frame),
      median_value = purrr::map_dbl(metric_id, \(metric) median(character_frame[[metric]], na.rm = TRUE))
    ) |>
    dplyr::left_join(
      catalog_check |>
  ...
  raw_values_long <- character_frame |>
  ...
  source_year_long <- character_frame |>
  ...
  character_model_df <- character_frame |>
  ...
  list(
    ...
    character_model_df = character_model_df
  )
}
