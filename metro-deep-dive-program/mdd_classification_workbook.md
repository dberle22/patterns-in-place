...
| Corridor Intelligence method | Core engine logic for what counts as a corridor candidate | Start with Intelligence Framework zones plus proximity; POI, infrastructure, and trend evidence can enter as optional inputs |
...
| Act 4 / Zone Corridors / Corridor Intelligence method | Explanation | Corridors using the future Corridor Intelligence Engine | Defines how tract-derived zones become meaningful higher-order groupings | Reusable engine; market-specific outputs | In progress | Distinguish from the national zone model: corridors are selected related groupings, not necessarily strictly contiguous |
...
| Corridors | Internal structure outputs, Q4 access work, POI and Infrastructure Engine outputs, possible trend overlays, and any future zone/corridor method notes | Corridor candidate pools, selected corridor summaries, corridor-level theses, Act 4 stat-block inputs | Zone-composition outliers and related internal-structure signals | Future Corridor Intelligence Engine extending the Intelligence Framework; setup-heavy because the method is still being defined and consumes several earlier components |
...
| Internal structure | Shared geo mart and rollups | shared dataset / mart | Supports tract, ZCTA, place, and related geography identities and declared relationships from one core layer | Exists; consumer adoption pending | `silver.dim_geo`, `mart_geography`, and geography-engine contracts | Yes | Yes | Exact rollups are ready; metric allocation and map materialization remain consumer-driven |
...
| Q2 Job-proximity gradient | Infrastructure Engine | engine | Supplies governed road and other physical-network features for methods that move beyond pure geographic proximity | Partial | Richmond/Jacksonville OSM extracts and normalization code | Yes | Maybe | Initial contract stops before routing or travel-time modeling |
...
| Q4 Daily-needs access | POI Engine | engine | Supplies classified, provenance-rich Richmond place points with tract/county assignment and source-address postal-ZIP evidence | Ready for first analysis | `engines/poi/` Epics 1–5: Overture acquisition, normalization, exact governed mappings, QA, and Richmond assignments | Yes | Yes | Point preparation is reusable; Q4 decides the daily-needs basket and access method. Postal ZIP is not a Census ZCTA assignment; governed ZCTA geometry remains a Geography dependency. |
| Q4 Daily-needs access | Infrastructure Engine | engine | Supplies roads and other physical features when the access method needs them | Partial | Richmond/Jacksonville OSM and `osmextract` builds | Yes | Maybe | Infrastructure remains descriptive input until an analysis defines network or barrier behavior |
...
| Corridors | Corridor Intelligence Engine | engine | Extends Intelligence Framework zone outputs into candidate corridor groupings using proximity and optional POI, infrastructure, and trend evidence | Partial | Phase 7 Stage 2 proposal plus internal structure and access inputs | Yes | Maybe | Separate future engine; distinct from the national zone model and from editorial corridor naming |
...
| Shared geo mart and rollups | shared dataset / mart | Tract, ZCTA, place, regional, and other geography identities, joins, labels, and declared relationship types across the program | High | Exists; consumer adoption pending | Strong | `silver.dim_geo` plus `mart_geography` | foundations | Exact rollups are ready; allocation/harmonization use and display geometry remain on-demand |
...
| POI Engine | engine | Classified, provenance-rich, geographically assigned place points for Q4 and later analyses | High | Partial | Strong | Reuse Richmond/Jacksonville Overture ingests and Stoop governance patterns | shared platform asset | Owns points and taxonomy inputs, not catchments, access interpretation, barriers, or corridors |
...
| Infrastructure Engine | engine | Governed roads, rail, waterways, airports, ports, and other physical-network geometry | Medium | Partial | Strong | Reuse Richmond/Jacksonville OSM extraction and normalization work | shared platform asset | Stops before routing, barrier interpretation, catchments, and corridors |
| Corridor Intelligence Engine | engine | Corridor candidate pools, selected corridor summaries, and corridor stat-block inputs | Medium | Partial | Medium | Extend Intelligence Framework zones with proximity and optional access, infrastructure, and trend evidence | shared platform asset | Separate future engine; editorial corridor names remain issue-owned |
...
| Corridor ownership | Treat Corridor Intelligence as a separate future Intelligence Framework extension | It can combine zone, proximity, POI, infrastructure, and trajectory evidence without widening the POI or Infrastructure engines |
