-- Materialize the first reusable CBSA-grain benchmark metric surface.
-- This is the narrow metric layer that the benchmarking engine can summarize
-- repeatedly across national, regional, division, state, and peer-set cuts.

CREATE SCHEMA IF NOT EXISTS mart_benchmarking;

CREATE OR REPLACE TABLE mart_benchmarking.benchmark_cbsa_metrics AS
WITH raw_metric_surface AS (
  SELECT
    ...
    m.metric_id,
    m.metric_label,
    CAST(m.metric_value AS DOUBLE) AS value,
    CAST(m.source_year AS INTEGER) AS year,
    'mart_intelligence.intelligence_character' AS source_name,
    CONCAT('Character source year ', CAST(m.source_year AS VARCHAR), '.') AS vintage_note
  ...
  SELECT
    l.cbsa_code AS geo_id,
    l.cbsa_name AS geo_name,
    'cbsa' AS geo_level,
    ...
    CAST(m.metric_value AS DOUBLE) AS value,
    CAST(m.source_year AS INTEGER) AS year,
    'mart_intelligence.intelligence_livability' AS source_name,
    CONCAT('Livability source year ', CAST(m.source_year AS VARCHAR), '.') AS vintage_note
  ...
  SELECT
    o.cbsa_code AS geo_id,
    o.cbsa_name AS geo_name,
    'cbsa' AS geo_level,
    ...
    CAST(m.metric_value AS DOUBLE) AS value,
    CAST(m.source_year AS INTEGER) AS year,
    'mart_intelligence.intelligence_opportunity' AS source_name,
    CONCAT('Opportunity source year ', CAST(m.source_year AS VARCHAR), '.') AS vintage_note
  ...
  SELECT
    cf.cbsa_code AS geo_id,
    cf.cbsa_name AS geo_name,
    'cbsa' AS geo_level,
    'cross_frame' AS frame_id,
    'identity' AS theme_group,
    m.metric_id,
    m.metric_label,
    CAST(m.metric_value AS DOUBLE) AS value,
    CAST(NULL AS INTEGER) AS year,
    'mart_intelligence.intelligence_cross_frame' AS source_name,
    'Current Intelligence Framework promoted percentile surface.' AS vintage_note
  FROM mart_intelligence.intelligence_cross_frame AS cf
  CROSS JOIN LATERAL (
    VALUES
      ('character_percentile_rank', 'Character percentile rank', cf.character_percentile_rank),
      ('livability_percentile_rank', 'Livability percentile rank', cf.livability_percentile_rank),
      ('opportunity_percentile_rank', 'Opportunity percentile rank', cf.opportunity_percentile_rank),
      ('cross_frame_percentile_rank', 'Cross-frame percentile rank', cf.cross_frame_percentile_rank)
  ) AS m(metric_id, metric_label, metric_value)
),
deduped_surface AS (
  SELECT
    s.geo_level,
  ...
  FROM (
    SELECT
      s.*,
      ROW_NUMBER() OVER (
        PARTITION BY s.geo_level, s.geo_id, s.metric_id, s.year
        ORDER BY
          CASE s.frame_id
            WHEN 'cross_frame' THEN 1
            WHEN 'character' THEN 2
            WHEN 'livability' THEN 3
...
SELECT
  ...
  c.unit,
  c.value_direction,
  c.preferred_summary_stat,
  s.metric_label,
  s.value,
  s.year,
  s.source_name,
  s.vintage_note
FROM deduped_surface AS s
LEFT JOIN mart_benchmarking.benchmark_metric_catalog AS c
  ON s.metric_id = c.metric_id;
